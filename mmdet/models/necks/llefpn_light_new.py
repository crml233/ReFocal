...
class Li_LLEFPN(BaseModule):
    ...
    def __init__(self,
        ...
        if self.start_level == 0:
            self.extract_for_p2 = ConvModule(
                in_channels[0],
                out_channels,
                3,
                padding=1,
        ...
        #不管使不使用p2，avepool都是这几层(0,1,2)
        for i in range(0, self.backbone_end_level-1):    #start_level =1,  backbone_end_level=4
            #for m2,p3,p4,p5
            ...
            self.pools.append(extract_and_pool)
       

        for i in range(1, self.backbone_end_level):    #start_level =1,
            #for m1,m2,m3
    ...
    def forward(self, inputs):
        ...
        if self.add_lle_convs == 'on_output':
            #这里的0层相当于C2
            if self.start_level == 0 and self.extract_p2:
                P2_extract = self.extract_for_p2(laterals[0])
            else:
                P2_extract = laterals[0]
            ME_0 = self.pools[0](P2_extract) #现在大小和M_1一样
