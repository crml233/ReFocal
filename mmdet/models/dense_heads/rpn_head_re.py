...
from mmdet.core import bbox_overlaps, multi_apply, images_to_levels, anchor_inside_flags,unmap
...
class RPNHead(AnchorHead):
    ...
    def loss(self,
             ...
             bbox_preds,
             gt_bboxes,
            #  gt_labels = None,#rpn的gt label就是none
             img_metas,
        ...
        cls_reg_targets = self.get_targets(
            ...
            gt_labels_list= None,#gt_labels,
            label_channels=label_channels)
        if cls_reg_targets is None:
            return None
        (labels_list, label_weights_list, bbox_targets_list, bbox_weights_list,
         num_total_pos, num_total_neg, pos_preds_gt, pos_num_gt) = cls_reg_targets
        num_total_samples = (
    ...
    def loss_single(self, cls_score, bbox_pred, anchors, labels, label_weights,
        ...
        pos_preds_gt = pos_preds_gt.reshape(-1)
        pos_num_gt = pos_num_gt.reshape(-1)
        ...
        flatten_pos_num_gt = pos_num_gt#torch.cat(pos_num_gt)
        reverse_focal_rate = pos_preds_gt
        ...
        margin_factor = torch.zeros_like(labels).float()
    ...
    def _get_targets_single(self,
        ...
        assigned_gt_inds =  sampling_result.pos_assigned_gt_inds #min_area_inds[pos_inds] # assigned gt inds
        ...
        pos_decoded_bbox_preds = self.bbox_coder.decode(sampling_result.pos_bboxes, pos_bbox_preds)
        pos_decoded_target_preds = self.bbox_coder.decode(sampling_result.pos_bboxes,pos_bbox_targets)
        ...
        cls_iou = pos_cls_score #+iou_pred
        ...
        for i in range(num_gts):
            if assigned_gt_inds.size(0)>0:
                ...
                gt_i_pos_num = torch.sum(gt_i_pos_inds).float()
        ...
        if unmap_outputs: #True
            # unmap函数的作用是创建一个形状为[num_total_anchors, *]
            # 的新张量，并将bbox_weights的值填充到被选中的锚点的位置，未被选中的锚点的权重被设置为0。
            ...
            pos_preds_gt = unmap(pos_preds_gt, num_total_anchors, inside_flags)
            pos_num_gt = unmap(pos_num_gt, num_total_anchors, inside_flags)
        



        return (labels, label_weights, bbox_targets, bbox_weights, pos_inds,
                neg_inds, sampling_result , pos_preds_gt, pos_num_gt)
    ...
    def get_targets(self,
        ...
        all_pos_preds_gt, all_pos_num_gt = rest_results
        # no valid anchors
