...
class RFLA_FCOSHead_Refocal(AnchorFreeHead):
    ...
    def loss(self,
        ...
        all_level_points = self.get_points(featmap_sizes, bbox_preds[0].dtype,
                                           bbox_preds[0].device)
        labels, bbox_targets, pos_preds_gt, pos_num_gt, centerness_per_gt = self.get_targets(all_level_points, gt_bboxes,
                                                gt_labels)
       ...
       ############################ reverse focal loss
        ...
        #sum_pos_preds_gt = torch.sum(flatten_pos_preds_gt)
        #reverse_focal_rate = flatten_pos_preds_gt/(sum_pos_preds_gt+1e-6)
        reverse_focal_rate = flatten_pos_preds_gt

        pos_num_gt_inds = flatten_pos_num_gt.nonzero()
        ...
        if self.rfl:
            loss_cls = self.loss_cls(
                flatten_cls_scores, flatten_labels, reverse_focal_rate=reverse_focal_rate.detach(), margin_factor=margin_factor.detach(), avg_factor=num_pos)        
        else:
            loss_cls = self.loss_cls(
        ...
        pos_centerness_targets = flatten_centerness_per_gt[pos_inds]
    ...
    def get_targets(self, points, gt_bboxes_list, gt_labels_list):
        ...
        labels_list, bbox_targets_list,pos_preds_gt_list, pos_num_gt_list,  centerness_per_gt_list  = multi_apply(
        ...
        centerness_per_gt_list = [centerness_per_gt.split(num_points, 0) for centerness_per_gt in centerness_per_gt_list]
        ...
        for i in range(num_levels):
            ...
            centerness_per_gt = torch.cat([centerness_per_gt[i] for centerness_per_gt in centerness_per_gt_list])
            concat_lvl_centerness_per_gt.append(centerness_per_gt)
            
        return concat_lvl_labels, concat_lvl_bbox_targets, concat_lvl_preds_gt,  concat_lvl_num_gt, concat_lvl_centerness_per_gt

    def _get_target_single(self,cls_scores, bbox_preds, gt_bboxes, gt_labels, points, rfields, regress_ranges,
                           num_points_per_lvl):
        ...
        if self.if_qkld:
            ...
            reverse_cls_iou = (reverse_cls_iou**self.qkld)*(kld_pred**(1-self.qkld))
        

        ######
        pos_preds_gt = torch.zeros_like(labels).float()
        pos_num_gt = torch.zeros_like(labels).float()
        centerness_per_gt = torch.zeros_like(labels).float()
        for i in range(num_gts):
                if assigned_gt_inds.size(0)>0:
                    ...
                    #将pos_centerness_targets用其最大值进行归一化 
                    if pos_centerness_targets[gt_i_pos_inds].numel() > 0:
                        pos_centerness_targets[gt_i_pos_inds] = pos_centerness_targets[gt_i_pos_inds]/torch.max(pos_centerness_targets[gt_i_pos_inds])
                    centerness_per_gt[pos_inds[gt_i_pos_inds]] = pos_centerness_targets[gt_i_pos_inds]
        ...
        return labels, bbox_targets, pos_preds_gt, pos_num_gt, centerness_per_gt
